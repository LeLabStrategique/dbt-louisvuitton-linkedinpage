...
WITH seniority_mapping AS (
    SELECT
      ...
      CAST(f.seniority_id AS INT64) AS seniority_id,
      ...
      ROW_NUMBER() OVER (
        PARTITION BY CAST(f.seniority_id AS INT64), DATE(f._fivetran_synced)
        ORDER BY f._fivetran_synced DESC
      ) AS rn

    FROM 
      {{ source('linkedin_pages_normalized', 'followers_by_seniority') }} f
    LEFT JOIN 
