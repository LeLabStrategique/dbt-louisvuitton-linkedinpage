...
WITH industry_mapping AS (
    SELECT
      ...
      CAST(f.industry_id AS INT64) AS industry_id,
      COALESCE(fn.name, 'Unknown') AS industry_name,
      ...
      ROW_NUMBER() OVER (
        PARTITION BY CAST(f.industry_id AS INT64), DATE(f._fivetran_synced)
    ...
    LEFT JOIN 
      {{ source('linkedin_pages_normalized', 'industry') }} fn
