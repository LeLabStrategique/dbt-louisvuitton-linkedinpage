...
WITH dimension_mapping AS (
    SELECT
      ...
      -- Le champ 'geo_id' est casté en INT64
      CAST(f.geo_id AS INT64) AS geo_country_id,
      COALESCE(m.value, 'Unknown') AS geo_country_name,
      ...
      -- Logique de déduplication : prend le dernier enregistrement par jour/pays (ROW_NUMBER)
      ROW_NUMBER() OVER (PARTITION BY CAST(f.geo_id AS INT64), DATE(f._fivetran_synced) ORDER BY f._fivetran_synced DESC) AS rn
      
    FROM 
      {{ source('linkedin_pages_normalized', 'page_statistic_by_geo_country') }} f
    LEFT JOIN 
